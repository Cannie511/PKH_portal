class Hrm0900Controller{
    ...
    $onInit(){
        ...
        let previousSearch = sessionStorage.hrm0900;
    ...
    loadInit() {
        let param = {};
        this.API.service('init-data', this.API.all('hrm0900'))
            .post(param)
            .then((response) => {
                let data = response.plain().data;
                data.listYear.forEach(item => {
                    item.year = `${item.year}`;
                });
                this.m.init = data;
            });
    ...
    doSearch(page) {
        ...
        let searchService = this.API.service('search', this.API.all('hrm0900'));
        param.page = page;
    ...
    download() {
        let param = angular.copy(this.m.filter);
        let service = this.API.service('download', this.API.all('hrm0900'));
        service.post(param)
            .then((response) => {
