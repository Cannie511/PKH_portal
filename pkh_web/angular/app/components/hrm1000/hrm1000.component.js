class Hrm1000Controller{
    ...
    $onInit(){
        ...
        let previousSearch = sessionStorage.hrm1000;
    ...
    doSearch(page) {
        ...
        let searchService = this.API.service('search', this.API.all('hrm1000'));
        param.page = page;
    ...
    download() {
        let param = angular.copy(this.m.filter);
        let service = this.API.service('download', this.API.all('hrm1000'));
        service.post(param)
            .then((response) => {
