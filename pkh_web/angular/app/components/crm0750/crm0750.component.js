import { Crm0750DialogController } from './crm0750.dialog';
class Crm0750Controller {
    ...
    init(){
        let previousSearch = sessionStorage.crm0750;
        this.loadInit();
    ...
    loadInit() {
        let service = this.API.service('load-promotion', this.API.all('crm0750'));
        service.post()
            .then((response) => {
    ...
    doSearch(index,page) {
        // Get list product 
        let searchService = this.API.service('search', this.API.all('crm0750'));
    ...
    download() {
        ...
        let service = this.API.service('download', this.API.all('crm0750'));
        param.down = 1;
        service.post(param)
            .then((response) => {
    ...
    notifyZalo(payment_id){
        ...
        swal({
            ...
            let service = self.API.service('notify-zalo', self.API.all('crm0750'));
            service.post(param)
                .then((res) => {
    ...
    update_accountant(store_id, store_name) {
        ...
        modalOption = {
            size: 'dialog-768',
            controller: Crm0750DialogController,
            resolve: {
                param: param
            }
        };
