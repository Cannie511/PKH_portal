class Hrm1010Controller{
     ...
     */
    loadInitData() {
        let service = this.API.service('init-data', this.API.all('hrm1010'));
        service.post({})
            .then((response) => {
     ...
     */
    load(id) {
        let service = this.API.service('load', this.API.all('hrm1010'));
        let param = { id: id };
        service.post(param)
            .then((response) => {
    ...
    save(isValid, form) {
        if(this.validate(this.m.form, form)) {
            ...
            let service = this.API.service('save', this.API.all('hrm1010'));
            service.post(param)
                .then((response) => {
    ...
    publish() {
        if (this.m.init.id > 0 ) {
            let service = this.API.service('publish', this.API.all('hrm1010'));
            let param = angular.copy(this.m.form);
            service.post(param)
                    .then((response) => {
                        let result = response.data.data;
                        if( result.rtnCd == true ) {
                            this.ClientService.success(result.msg);
                            this.RouteService.goState('app.hrm1000');
                        } else {
                            this.ClientService.error(result.msg);
                        }
                    }, (response) => {
    ...
    delete() {
        if( this.m.init.id > 0) {
            swal({
                ...
                let API = this.API
                let searchService = API.service('delete', API.all('hrm1010'));
                let param = {
                        id: this.m.init.id
                    }
