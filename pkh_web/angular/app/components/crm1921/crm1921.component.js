class Crm1921Controller {
    ...
    doSearch(page) {
        ...
        // Get list product 
        let searchService = this.API.service('search', this.API.all('crm1921'));
    ...
    download() {
        let param = angular.copy(this.m.filter);
        let service = this.API.service('download', this.API.all('crm1921'));
        param.down = 1;
        service.post(param)
            .then((response) => {
