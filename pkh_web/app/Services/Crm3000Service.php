...
class Crm3000Service extends BaseService
{
    ...
    {
        $sqlParam = array();
        $sql = "SELECT SUM(total_with_discount)/12 avg_total 
    ...
    public function getRetentionParam($param)
    {
        $sqlParam = array();
        $sql = "
            select 
                store_id, 
                min(order_date), 
                timestampdiff(year, min(order_date), 
                curdate()) retention 
                from trn_store_order
                where 1=1 ";
        $sql .= $this->andWhereString($param, 'store_id', 'store_id', $sqlParam);
