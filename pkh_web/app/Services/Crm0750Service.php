...
class Crm0750Service extends BaseService
{
    /**
     * @param $param
     * @return mixed
    ...
    public function download($param)
    {
        ...
        Excel::create($fileName, function ($excel) use ($data) {
            $excel->sheet('CongNo', function ($sheet) use ($data) {
                $sheet->loadView('admin.excels.crm0750-list')
                    ->with('data', $data);
            });
        })->store($ext, $path);
    ...
    public function selectZaloNotifyList($param)
    {
        ...
        $sql      = "
            ...
            left join trn_payment_advance c on c.payment_id = a.payment_id
...
//     order by a.payment_date desc, a.updated_at desc
    ...
    {
        DB::update('update trn_payment_advance set payment_sts = 1 where payment_id = ?', array($payment_id));
    }
